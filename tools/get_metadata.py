...
@mcp.tool()
def get_metadata(layer_ids: list[str]):
    """Résout les métadonnées CSW d'une liste de données. Retourne un dictionnaire layer_id -> {title, abstract, date, wfs_url, wfs_name, obsolete}."""
    ...
    for layer_id in layer_ids:
        ...
        if layer is None:
            results[layer_id] = {"error": f"Aucune donnée trouvée avec l'id '{layer_id}'"}
            continue
        ...
        try:
            ...
            layer['wfs_url'] = None
            results[layer_id] = {"error": "Serveur CSW inaccessible (timeout)", "wfs_url": None}
            continue
        except httpx.HTTPStatusError as e:
            layer['wfs_url'] = None
            results[layer_id] = {"error": f"Erreur HTTP CSW: {e.response.status_code}", "wfs_url": None}
            continue
        except httpx.RequestError as e:
            layer['wfs_url'] = None
            results[layer_id] = {"error": f"Erreur réseau CSW: {e}", "wfs_url": None}
            continue
        except Exception as e:
            layer['wfs_url'] = None
            results[layer_id] = {"error": f"XML CSW invalide: {e}", "wfs_url": None}
            continue
        ...
        if record is None:
            layer['wfs_url'] = None
            results[layer_id] = {"error": "Format CSW non supporté (pas de csw:Record)", "wfs_url": None}
            continue
